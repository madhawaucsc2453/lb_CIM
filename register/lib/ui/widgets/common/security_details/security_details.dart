...
class SecurityDetails extends StackedView<SecurityDetailsModel> {
 ...
 final Map<int,Map<String, String>> formData;
  ...
  void onViewModelReady(SecurityDetailsModel viewModel) {
    ...
    viewModel.securityQuestionController.text = formData[1]!['securityQuestion']!;
  ...
  decoration: InputDecoration(
    ...
    suffixIcon: IconButton(
      ...
      icon: Icon(
        viewModel.visiblebool ? Icons.visibility : Icons.visibility_off,
      ),
    ),
  ),
  ...
  onChanged: (value) => onFieldChanged(
    ...
    value,
          ...
          ElevatedButton(
            ...
            viewModel.isPasswordValid && viewModel.isConfirmPasswordValid && viewModel.isSecurityQuestionValid ?()=>ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Please fill all the fields'),
              ),
            ):()=>ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Password, Confirm Password and Security Question are required'),
              ),
            ),
